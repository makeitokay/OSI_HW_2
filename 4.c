...
int main(int argc, char* argv[]) {
    ...
    ftruncate(shm, 32);
    float* lawyer_distribution = mmap(NULL, 32, PROT_READ | PROT_WRITE, MAP_SHARED, shm, 0);

    char* sem_names[8] = {"sem_1", "sem_2", "sem_3", "sem_4", "sem_5", "sem_6", "sem_7", "sem_8"};
        ...
        if (pid == 0) {
            ...
            if (rnd >= 8) {
                ...
                if (rnd2 == 0) {
                    ...
                    // отдаем деньги другому наследнику только если ему уже распределено, если нет - то поступаем честно
                    if (lawyer_distribution[rnd_person] != -1) {
                        printf("[info]: Адвокат отсыпает часть денег от %d наследника %d наследнику\n", i + 1, rnd_person + 1);
